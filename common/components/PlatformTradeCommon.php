...
// 所有平台交易类的
class PlatformTradeCommon {

    // 交易不中 USD币本位 USDT 美元本位
    public  $base_coin = 'USD';

    public static $base_buy_num = 1; //最开始购买数量
    // 盈利比例
    ...
    // 补仓间隔
    public static $add_distance = 0.003;
    ...
    public function returnAllTableName($coin =''){
        ...
        $all_table_name = ['','sea_okex_buy_up_order','sea_okex_buy_down_order','sea_okex_sell_up_order','sea_okex_sell_down_order','sea_okex_buy_up_order_double','sea_okex_buy_down_order_double','sea_okex_sell_up_order_double','sea_okex_sell_down_order_double'];
        if($coin=='xrp') {
    ...
    public function doTrade($type,$coin,$api_key_info,$admin_user_id){
        ...
        if($last_state == 2){
            ...
            if(!$sell_order_info){
                ...
                return $this->doSell($coin,$type,$last_info,$buy_table_name);
        ...
        if(in_array($last_state,[-2,-1])){
            ...
            $update_data = ['is_deleted'=>'Y','modify_time'=>date('Y-m-d H:i:s')] ;
    ...
    public function doSell($coin,$sell_type,$buy_order_info,$buy_table_name,$mark_price = ''){
        ...
        // 判断是否需要进行反向订单的操作
        $check_add_double = $this->checkAddDouble($buy_order_info,$sell_type,$mark_price) ;

        if($check_add_double){
            return $this->addDoubleOrder($buy_order_info,$sell_type);
        }

        // 更新反向购买订单的订单信息
        $this->updateBuyDoubleOrder($buy_order_info,$sell_type);

        //查询反向完成的订单
        $okex_model = new OkexOrder();
        $all_table_name = $this->returnBuySellTableName($sell_type);
        $buy_double_table_name = $all_table_name['buy_double_table_name'];
        $buy_double_complete_order_list = $okex_model->getCompletedOrderList($buy_double_table_name,$buy_order_info['group_id']);

        // 判断购买级别
        if($buy_order_info['level'] < self::$max_level || !$buy_double_complete_order_list){
            ...
            if($check_add_extra){
                ...
                if($check_stop){
                    ...
                    $this->addLog('DO_SELL_SELL_ORDER_STOP_997',$buy_order_id,$res);
                    ...
                    $this->addLog('DO_SELL_SELL_ORDER_NORMAL_1004',$buy_order_id,$res);
            ...
            //直接挂出符合条件的盈利的价格进行
            return  $this->doSellByNormalAndDouble($sell_type,$buy_order_info);
        }
    ...
    public function dealSellTradeBySuccessBuyAndSellOrderInfo($type,$buy_order_info,$sell_order_info,$buy_table_name,$sell_table_name){
        ...
        // 如何卖单已经成功不做任何处理
        $check_add_double = $this->checkAddDouble($buy_order_info,$type) ;

        if($check_add_double){
            return $this->addDoubleOrder($buy_order_info,$type);
        }

        // 实时查询当前购买的订单的状态的
        $this->doSellByNormalAndDouble($type,$buy_order_info) ;

        $this->updateBuyDoubleOrder($buy_order_info,$type);
        //处理所有的反向订单
        //$this->dealDoubleSellOrder($type,$buy_order_info,$sell_order_info);

        if($sell_order_info['state'] == 2){

            //判断是否有反向，有反向的直接对手价卖出
            $check_done_double = $this->checkDoneDouble($buy_order_info,$type,$sell_order_info);

            if($check_done_double){
        ...
        if(in_array($service_sell_order_state,[-2,-1])){
            ...
            return $this->doSell($coin,$sell_type,$buy_order_info,$buy_table_name);
        }
        ...
        if($service_sell_order_state == 0){
            ...
            if($check_add_double){
                ...
                // 查询对手单是否完成
                $model = new OkexOrder();
                $all_table_info = $this->returnBuySellTableName($type);
                $sell_double_table_name = $all_table_info['sell_double_table_name'];
                $group_id = $buy_order_info['group_id'];
                $sell_double_order_info = $model->getInfoByOrginalGroupId($sell_double_table_name,$group_id);
                $ext = time()-strtotime($sell_order_info['create_time']) ;
                $min_cancel_seconds = $this->returnCancelOrderSeconds();
                if($ext >= $min_cancel_seconds || $sell_double_order_info['state'] ==2){
                    //执行撤单操作
     ...
     * @return mixed
     */
    public function checkAddDouble($buy_order_info,$type,$mark_price = ''){

        //判断是否
        if ($buy_order_info['level'] < self::$max_level) {

            return false;
        }

        // 大于等于最大层级，同时最新的购买已经成功
        //-2:失败-1:撤单成功0:等待成交1:部分成交2:完全成交3:下单中4:撤单中
        $order_state =  $buy_order_info['state'] ;

        if(in_array($order_state,[0,1,3,4])){
            return false ;
        }
        ...
        $group_id = $buy_order_info['group_id'];
        $all_table_info = $this->returnBuySellTableName($type);
        $buy_table_name = $all_table_info['buy_table_name'];
        $buy_order_list = $model->getOrderListByGroupId($admin_user_id, $buy_table_name, $group_id);

        //判断是否已经购买
        $buy_double_table_name = $all_table_info['buy_double_table_name'] ;
        $buy_double_order_list = $model->getOrderListByGroupId($admin_user_id,$buy_double_table_name,$group_id);

        $buy_double_order_info = isset($buy_double_order_list[0])?$buy_double_order_list[0]:[];

        if($buy_double_order_info){
        ...
        if (!$mark_price) {
            $mark_price = $this->getCoinMarkPriceFromService($buy_order_info['coin']);
            $this->mark_price = $mark_price;
        }

        // 低于购买均价就需要挂委托单了
        $buy_price_avg = $this->getAvgPriceByList($buy_order_list);

        if($type =='up'){
            // 要低于对手价的最大购买数量的均价才会出发
            if($mark_price <= $buy_price_avg){
                return true ;
            }
        }else{
            if($mark_price >= $buy_price_avg){
     ...
     * @return mixed
     */
    public function addDoubleOrder($buy_order_info,$type){
        ...
        $total_buy_order_num = count($buy_order_list) ;
        // 获取第一个183818381838订单信息
        $first_order_info = $buy_order_list[$total_buy_order_num-1];
        $first_price_avg = $first_order_info['price_avg'] ;

        //由于OKEX的规则，反向单只能挂出一个单
        $buy_double_table_name = $all_table_info['buy_double_table_name'];

        $coin = $this->buy_coin ;

        $buy_double_total_price = 0 ;
        $buy_double_total_num = 0 ;

        // 新增委托
        $buy_type = $type =='up' ? 'down' :'up';
        ...
        for($i=1;$i<=self::$max_level;$i++){
            ...
            $size = $this->getBuySizeBuyLevel($level-1,$this->returnBuyNumByCoin($coin));
            $base_percent = self::$earn_percent;
            $buy_percent = $buy_type =='up' ? (1+$base_percent) : (1-$base_percent);
            $price = $first_price_avg*pow($buy_percent,($level-1));// 以起始买入价进行购买

            $buy_double_total_price = $buy_double_total_price + $price * $size ;
            $buy_double_total_num += $size ;

        }
        $buy_double_price_avg = $buy_double_total_price / $buy_double_total_num ;
        //*/
        ...
        $buy_order_list = $model->getCompletedOrderList($buy_table_name,$group_id) ;
        $buy_price_avg = $this->getAvgPriceByList($buy_order_list) ;

        if($buy_type =='up'){

                $buy_price = $buy_price_avg*(1-self::$earn_percent+0.001);
                $this->doBuyDouble($coin,$buy_type,$buy_double_total_num,$buy_price,$group_id,0,$buy_double_table_name);
        ...
        // 同时取消挂出的委托单
        $sell_table_name = $all_table_info['sell_table_name'] ;
        $sell_order_info = $this->checkIsSell($sell_table_name,$buy_order_info['order_id']);
        if($sell_order_info){
            $this->cancelByOrderId($sell_order_info['order_id']) ;
        }
     ...
     * @return mixed
     */
    public function doBuyDouble($coin,$buy_type,$size,$price,$group_id,$level,$buy_double_table_name){

        $otype = $buy_type == 'up' ? 1:2;
        $order_type = 0; //普通委托
        #TODO 为了加快反向订单的成交 直接按照对手价进行成交
        #TODO 由于对手价会导致损失 ，所以只能以委托价格成交
        $match_price = 0 ; // 是否以对手价进行购买1-是 0-否
        $obj = new SwapApi($this->returnConfig());
        $client_oid = 0 ;
        $instrumentId = $this->returnInstrumentId($coin);
        $leverage = $this->leverage;// 杠杆倍数
        ...
        $this->addLog('BUY_DOUBLE_1869_'.strtoupper($buy_type),$group_id,$res);
        ...
        if(!$error_code){
            ...
            return $this->addRowData($coin,$order_info,0,$group_id,$level,$buy_double_table_name);
     ...
     * @return mixed
     */
    public function doSellDoubleByMark($coin,$sell_type,$size,$price,$group_id,$sell_double_table_name){

        $otype = $sell_type == 'up' ? 3:4;
        $order_type = 0; //普通委托
        #TODO 为了加快反向订单的成交 直接按照对手价进行成交
        $match_price = 1 ; // 是否以对手价进行购买1-是 0-否
        $obj = new SwapApi($this->returnConfig());
        $client_oid = 0 ;
        $instrumentId = $this->returnInstrumentId($coin);
        $leverage = $this->leverage;// 杠杆倍数
        ...
        $this->addLog('SELL_DOUBLE_1910_'.strtoupper($sell_type),$group_id,$res);
        ...
        if(!$error_code){
            ...
            $this->addLog('QUERY_ORDER_1919_'.strtoupper($sell_type),$order_id,$order_info);

            return $this->addRowData($coin,$order_info,0,0,0,$sell_double_table_name,$group_id);

        }
    }

    /**
     * 销售普通和反向订单 根据趋势进行变更交易类型
     * @param $type
     * @param $buy_order_info
     * @return mixed
     */
    public function doSellByNormalAndDouble($type,$buy_order_info){
        ...
        $all_table_info = $this->returnBuySellTableName($type);
        $buy_table_name = $all_table_info['buy_table_name'] ;
        $buy_order_list = $model->getOrderListByGroupId($this->admin_user_id,$buy_table_name,$buy_order_info['group_id']);
        $first_buy_table_info = $buy_order_list[count($buy_order_list)-1];
        $buy_double_table_name = $all_table_info['buy_double_table_name'] ;
        $buy_double_list = $model->getOrderListByGroupId($this->admin_user_id,$buy_double_table_name,$buy_order_info['group_id']);

        $buy_double_order_info = isset($buy_order_list[0]) ? $buy_order_list[0]:[];
        ...
        $sell_order_info = $this->checkIsSell($sell_table_name,$buy_order_list[0]['order_id']);
        if($sell_order_info){
            // 判断是否需要取消订单
            $this->dealSellOrder($sell_table_name,$sell_order_info);
        }

        $sell_double_order_info = $this->checkIsSell($sell_double_table_name,$buy_double_list[0]['order_id']);
        if($sell_double_order_info){
            // 判断是否需要取消订单
            $this->dealSellOrder($sell_double_table_name,$sell_double_order_info);
        }

        $qushi_info = $this->getQushiAndSellPrice($type,$mark_price,$buy_price_avg,$buy_double_order_info);
        $sell_price  = $qushi_info['sell_price'] ;

        //判断是否已经卖出
        if(!$sell_order_info){
            //对手价卖出
            $buy_total_num =$this->getSellSizeByOrderList($buy_order_list)  ;
            if($buy_total_num >0){
                $this->onlySell($type,$coin,$sell_price,$buy_total_num,$buy_order_list[0]['order_id'],'','');
            }

        }

        $buy_double_total_num =  $buy_double_list ? $this->getSellSizeByOrderList($buy_double_list) : 0 ;

        if(!$sell_double_order_info){
            $type  = $type =='up' ?'down':'up';
            if($buy_double_total_num > 0){
                $this->onlySell($type,$coin,$sell_price,$buy_double_total_num,$buy_double_list[0]['order_id'],$sell_double_table_name,$group_id);
            }
     ...
     * @param $group_id
     */
    public function onlySell($sell_type,$coin,$sell_price,$size,$buy_order_id,$sell_double_table_name,$group_id){
        $otype = $sell_type == 'up' ? 3 : 4 ;
        $client_oid = 0 ;
        $instrumentId = $this->returnInstrumentId($coin) ;
        $order_type = 0 ; // 普通委托
        ...
        $this->addLog('DO_SELL_ORDER_2072',$buy_order_id,$res);
        ...
        if(!$error_code){
            ...
            // 新增记录
            return $this->addRowData($coin,$order_info,$buy_order_id,0,0,'',$sell_double_table_name,$group_id);
    ...
    /**
     * 通过购买列表返回所有的购买数量
     * @param $buy_order_list
     * @return mixed
     */
    public function getSellSizeByOrderList($buy_order_list){
        $num = 0 ;

        if($buy_order_list){
            foreach($buy_order_list as $v){
                $num += $v['filled_qty'];
            }
        }
     ...
     * @return mixed
     */
    public function getTotalCompletedNumByOrderList($order_list){
        $total = 0  ;
        if($order_list){
     ...
     * @param $group_id
     */
    public function doubleOnlySellByPrice($sell_type,$sell_price,$coin,$size,$buy_order_id,$sell_double_table_name,$group_id){
        $otype = $sell_type == 'up' ? 3 : 4 ;
        $client_oid = 0 ;
        $instrumentId = $this->returnInstrumentId($coin) ;
        $order_type = 0 ; // 普通委托
        ...
        $this->addLog('DO_SELL_SELL_ORDER_NORMAL_2105',$buy_order_id,$res);
     ...
     * @return mixed
     */
    public function dealSellOrder($sell_table_name,$sell_order_info){

        //-2:失败-1:撤单成功0:等待成交1:部分成交2:完全成交3:下单中4:撤单中
        if($sell_order_info['state'] ==0){
            $ext = time()-strtotime($sell_order_info['create_time']) ;
            if($ext >=$this->returnCancelOrderSeconds()){
                return $this->cancelByOrderId($sell_order_info['order_id']) ;
            }
        }

        $model = new OkexOrder();
        if(in_array($sell_order_info['state'],[-2,-1])){
            // 删除记录
            $update_data['is_deleted'] = 'Y';
            $update_data['modify_time'] = date('Y-m-d H:i:s');
            return $model->baseUpdate($sell_table_name,$update_data,'id=:id',[':id'=>$sell_order_info['id']]);
        }

        if($sell_order_info['state'] !=2){

            $service_order_info = $this->getInfoByOrderIdFromService($sell_order_info['order_id'],$sell_order_info['coin']);
            $state = $service_order_info['state'];
            $update_data['state'] =$state ;
            $update_data['modify_time'] = date('Y-m-d H:i:s');
            return $model->baseUpdate($sell_table_name,$update_data,'id=:id',[':id'=>$sell_order_info['id']]);
        }
    ...
    /**
     * 获取反向的最大购买的平均值
     * @param $first_buy_order_info
     * @param $type
     * @return mixed
     */
    public function buyDoubleMaxPriceAvg($first_buy_order_info,$type){

        // 首次购买的平均价
        $first_price_avg = $first_buy_order_info['price_avg'];

        // 新增委托
        $buy_type = $type =='up' ? 'down' :'up';

        $coin = $first_buy_order_info['coin'] ;
        ...
        return $buy_double_total_price/$buy_double_total_num ;
    }
     ...
     * @return mixed
     */
    public function checkDoneDouble($buy_order_info,$type,$sell_order_info){
        //判断是否双向的都完成
        if($sell_order_info['state'] != 2){
            return false ;
        }
        ...
        $all_table_name = $this->returnBuySellTableName($type);
        $buy_double_table_name = $all_table_name['buy_double_table_name'] ;

        $admin_user_id = $this->admin_user_id ;
        $group_id = $buy_order_info['group_id'];

        $buy_double_order_list  = $okex_model->getOrderListByGroupId($admin_user_id,$buy_double_table_name,$group_id);

        if(!$buy_double_order_list){
            return true ;
        }

        // 判断卖单信息
        $sell_double_table_name = $all_table_name['sell_double_table_name'];
        $sell_double_order_info = $okex_model->getInfoByOrginalGroupId($sell_double_table_name,$group_id);
        if(!$sell_double_order_info){
            return false ;
        }
        ...
        if(in_array($sell_double_order_info,[-2,-1])){
            // 执行购买操作
            //上次取消了，直接再按照之前的数量重新购买

            // 删除当条记录
            $model = new OkexOrder();
            $this->addLog('DELETE_RECORD_2434_'.$sell_double_table_name,$sell_double_order_info['order_id'],$sell_double_order_info);
            $update_data = ['is_deleted'=>'Y','modify_time'=>date('Y-m-d H:i:s')] ;
            return $model->baseUpdate($sell_double_table_name,$update_data,'id=:id',[':id'=>$sell_double_order_info['id']]);
        }

        $service_order_info = $this->getInfoByOrderIdFromService($sell_double_order_info['order_id'],$coin);

        // 实时读取服务对应的订单状态

        if(in_array($service_order_info['state'],[-2,-1,1,2,3,4])){
            //更新订单表的状态信息
            $model = new OkexOrder();
            ...
            $model->baseUpdate($sell_double_table_name,$update_data,'id=:id',[':id'=>$sell_double_order_info['id']]);
            return false ;
        }
        ...
        if($service_order_info['state'] == 0){
            ...
            // 给与90秒的缓冲期，看是否超过，超过了再进行取消操作
            $ext = time() - strtotime($sell_double_order_info['create_time']) ;
            $min_cancel_seconds = $this->returnCancelOrderSeconds();
            if($ext > $min_cancel_seconds){
                // 时间久还没有下单成功 直接撤销订单
                $this->doBuyCancel($sell_double_order_info['order_id'],$sell_double_order_info['coin']) ;
            }
    ...
    /**
     * 处理反向卖单的信息 前提是卖单还没解决掉 自己先按照自己的逻辑进行操作
     * @param $type
     * @param $buy_order_info
     * @param $sell_order_info
     * @return mixed
     */
    public function dealDoubleSellOrder($type,$buy_order_info,$sell_order_info){
        ...
        $buy_double_order_list = $model->getCompletedOrderList($buy_double_table_name,$group_id);
        if(!$buy_double_order_list){
            return false ;
        }

        $buy_double_total_num = $this->getTotalCompletedNumByOrderList($buy_double_order_list);
        ...
        if(!$sell_double_order_info){
            ...
            if($sell_order_info['state'] ==2){
                // 对手价售出 及时止损或者止盈
                return $this->onlySell($sell_type,$coin,0,$buy_double_total_num,$buy_double_order_list[0]['order_id'],$sell_double_table_name,$group_id);

            }

        }

        if(in_array($sell_double_order_info['state'],[-1,-2,2])){
            return true ;
        }

        if($sell_double_order_info['state'] ==0){
            $ext = time()-strtotime($sell_double_order_info['create_time']);
            if($ext>=$this->returnCancelOrderSeconds() || $sell_order_info['state'] ==2 ){
                //对手单完成直接取消订单
                return $this->cancelByOrderId($sell_double_order_info['order_id']) ;
            }
        }

        $service_order_info = $this->getInfoByOrderIdFromService($sell_double_order_info['order_id'],$coin);
        if(in_array($service_order_info['state'],[-1,-2])){
            $update_data['is_deleted'] = 'Y';
        }
        $update_data['state'] = $service_order_info['state'];
        $update_data['filled_qty'] = $service_order_info['filled_qty'];
        $update_data['price_avg'] = $service_order_info['price_avg'];
        $update_data['fee'] = $service_order_info['fee'];
        $update_data['modify_time'] = $service_order_info['modify_time'];
        return $model->baseUpdate($sell_double_table_name,$update_data,'id=:id',[':id'=>$sell_double_order_info['id']]);
     ...
     * @return mixed
     */
    public function getDoubleMaxPrice($type,$price_avg){
        if($type =='up'){
            $res = $price_avg*pow((1+self::$earn_percent),self::$max_level+1 );
        }else{
            $res = $price_avg*pow((1-self::$earn_percent),self::$max_level+1 );
        }
     ...
     * @param $mark_price
     * @param $buy_price_avg
     * @param $buy_double_order_info
     * @return bool
     */
    public function getQushiAndSellPrice($type,$mark_price,$buy_price_avg,$buy_double_order_info){
        ...
        if($type =='up'){
            if($mark_price >= $buy_price_avg){
                ...
                // 判断当前价格是否大于买入最大的极限的价格
                if($buy_double_order_info){
                    $sell_price = $buy_double_order_info['price_avg']*(1+self::$earn_percent) ;
                }else{
                    $sell_price = $buy_price_avg*(1+self::$earn_percent) ;
                }
                $qushi = 'up';

            }else{
                //反向盈利
                if($mark_price <=$buy_double_order_info['price_avg']){
                    $sell_price = $buy_price_avg*(1-self::$earn_percent);
                    $qushi = 'down';
                }
            }
        ...
        return compact('qushi','sell_price');
    }
     ...
     * @return mixed
     */
    public function updateBuyDoubleOrder($buy_order_info,$sell_type){

        $all_table_name = $this->returnBuySellTableName($sell_type);
        $buy_double_table_name = $all_table_name['buy_double_table_name'];
        $buy_table_name = $all_table_name['buy_table_name'];
        $okex_model = new OkexOrder();
        $group_id = $buy_order_info['group_id'];
        $admin_user_id = $this->admin_user_id;
        $order_info = $okex_model->getOrderListByGroupId($admin_user_id,$buy_double_table_name,$group_id);
        $order_id = $order_info ? $order_info['order_id'] : 0 ;
        if(!$order_id){
            return false ;
        }
        $order_service_info = $this->getInfoByOrderIdFromService($order_id,$buy_order_info['coin']) ;

        $update_data['state'] = $order_service_info['state'];
        if(in_array($update_data['state'],[-1,-2])){
            $update_data['is_deleted'] = 'Y';
        }
        $update_data['filled_qty'] = $order_service_info['filled_qty'];
        $update_data['price_avg'] = $order_service_info['price_avg'];
        $update_data['modify_time'] = date('Y-m-d H:i:s') ;
        $okex_model->baseUpdate($buy_double_table_name,$update_data,'id=:id',[':id'=>$order_info['id']]);

        if($order_service_info['state'] ==2){

            $sell_table_name = $all_table_name['sell_table_name'];
            $sell_order_info = $this->checkIsSell($sell_table_name,$buy_order_info['order_id']);
            // 判断是否卖出
            $sell_double_table_name = $all_table_name['sell_double_table_name'];
            $is_sell = $this->checkIsSell($sell_double_table_name,$order_info['order_id']);
            if(!$is_sell){
                $type  = $sell_type =='up' ?'down':'up';
                $buy_double_total_num = $order_info['filled_qty'];
                $coin = $order_info['coin'];
                if($sell_order_info && $sell_order_info['state'] ==2){
                    //对手价卖出
                    $this->onlySell($type,$coin,0,$buy_double_total_num,$order_id,$sell_double_table_name,$group_id);
                }else{
                    //按照趋势卖出
                    $mark_price = $this->mark_price ;
                    $buy_order_list = $okex_model->getOrderListByGroupId($this->admin_user_id,$buy_table_name,$buy_order_info['group_id']);
                    $buy_price_avg = $this->getAvgPriceByList($buy_order_list);
                    $qushi_info = $this->getQushiAndSellPrice($sell_type,$mark_price,$buy_price_avg,$order_info);
                    $sell_price = $qushi_info['sell_price'] ;


                    $this->onlySell($type,$coin,$sell_price,$buy_double_total_num,$order_id,$sell_double_table_name,$group_id);

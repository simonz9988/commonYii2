...
class PlatformTradeCommonV3
{
    ...
    // 盈利比例
    public  $earn_percent = 0.004;
    // 止损比例
    public  $stop_percent = 0.04;
    ...
    // 补仓间隔
    public $add_distance = 0.002;

    //最大补仓间隔
    public $max_add_distance = 0.02;

    //购买类型
    public $buy_num_type = 'beishu';
    ...
    public function doTrade($type,$coin,$api_key_info,$admin_user_id){
        ...
        $sell_double_order_list = $okex_model->getListByOrginalGroupId($sell_double_table_name,$group_id) ;

        $this->buy_order_list = $buy_order_list ;
        $this->sell_order_list = $sell_order_list ;
        $this->buy_double_order_list = $buy_double_order_list ;
        $this->sell_double_order_list = $sell_double_order_list ;
        ...
        $this->updateOrderListLastedInfo($buy_double_table_name,$buy_double_order_list) ;
        $this->updateOrderListLastedInfo($sell_double_table_name,$sell_double_order_list) ;
    ...
    public function getBuyQushi(){
        ...
        $buy_double_order_info = $this->getLastedCompleteOrderByOrderList($buy_double_order_list);
        $buy_double_order_level = $buy_double_order_info ? $buy_double_order_info['level']:0 ;
        $temp_total_order_list = array_merge($buy_order_list,$buy_double_order_list) ;

        $total_order_list = [] ;
        //返回所有已完成的订单列表
        if($temp_total_order_list){
            foreach($temp_total_order_list as $v){
                if($v['state'] ==2){
                    $total_order_list[] = $v ;
                }
            }
        ...
        // 按照价格的从高到低进行排序
        $total_order_list = array_sort($total_order_list,'price_avg','desc') ;
        $total_order_num = count($total_order_list) ;
        $highest_order_info = isset($total_order_list[0]) ? $total_order_list[0] : [];
        $lowest_order_info =isset($total_order_list[$total_order_num-1]) ? $total_order_list[$total_order_num-1] : [];

        if($buy_order_level >= $this->max_level && $buy_double_order_level >= $this->max_level){

            $percent = 0 ;
            if($this->mark_price > $highest_order_info['price_avg']){

                $percent = $this->mark_price/$highest_order_info['price_avg'] - 1;
            }

            if($this->mark_price < $lowest_order_info['price_avg']){
                $percent = $lowest_order_info['price_avg']/$this->mark_price - 1;
            }

            if($percent > 0  && $percent >=$this->stop_percent){
                return 'STOP';
            }
        ...
        if($this->type =='down'){
            ...
            $buy_double_level = $buy_double_order_info ? $buy_double_order_info['level'] : 0 ;

            // 判断是否已经超过最大购买 需要预留一层用来对冲掉
            if(!$buy_double_level){
                if($buy_order_level < $this->max_level -1) {

                    // 判断档期那价格是否已经大于最后一个购买定案的指定额度
                    if($this->mark_price >$buy_order_info['price_avg']){

                        if( ($this->mark_price/$buy_order_info['price_avg'] -1) >=$this->add_distance){
                            return 'BUY_DOWN';
                        }
                    }
                }
                ...
                if($mark_price > $highest_order_info['price_avg']  && $buy_double_order_info['level'] < $this->max_level){

                    if( ($mark_price/$highest_order_info['price_avg']-1) >= $this->add_distance){
                        if($buy_double_level <=$buy_order_info['level']){
                            return  'BUY_UP';
                        }
                    }
                }

                if($mark_price < $lowest_order_info['price_avg']  && $buy_order_info['level'] < $this->max_level){

                    if( ($lowest_order_info['price_avg']/$mark_price -1) >= $this->add_distance){
                        if($buy_order_info['level'] <=$buy_double_level){
                            return  'BUY_DOWN';
                        }
                    }
                }
            ...
            if( !$buy_double_level){
                ...
                if($buy_order_level < $this->max_level - 1){
                    if($this->mark_price < $buy_order_info['price_avg']){
                        ...
                        if( ($buy_order_info['price_avg']/$this->mark_price-1) >= $this->add_distance){
                ...
                if($mark_price > $highest_order_info['price_avg']  && $buy_order_info['level'] < $this->max_level){
                ...
                if($mark_price < $lowest_order_info['price_avg']  && $buy_double_level < $this->max_level){
        ...
        // 判断触发止损
        if($buy_order_info && $buy_double_order_info && $buy_order_info['level'] == $buy_double_order_info['level'] && $buy_order_info['level'] >= $this->max_level){
            $percent = 0 ;
            if($mark_price > $highest_order_info['price_avg'] ){
                $percent = $mark_price/$highest_order_info['price_avg'] -1;
            }else if($mark_price < $lowest_order_info['price_avg']){
                $percent = $lowest_order_info['price_avg']/$mark_price  -1 ;
            }

            if($percent >=$this->stop_percent){
                return 'STOP';
            }
        }
    ...
    public function doBuyByQushi($qushi,$coin){
        ...
        if($this->type =='up'){
            ...
            if($qushi =='BUY_UP'){
                ...
                $buy_double_table_name = $this->buy_double_table_name ;
                $buy_order_list = $this->buy_double_order_list ;
            }
        }else{
            if($qushi =='BUY_UP'){
                $buy_order_list = $this->buy_double_order_list ;
        ...
        $first_buy_order_info = $this->getLastedOrderByOrderList($this->buy_order_list);
        $group_id = $first_buy_order_info['group_id'];
        $buy_order_info = $this->getLastedOrderByOrderList($buy_order_list) ;

        $buy_order_level = $buy_order_info ? $buy_order_info['level'] : 0 ;
        //反向订单购买订单是由高到低进行购买
        $buy_level = $buy_double_table_name ? ($this->max_level  - $buy_order_level) : $buy_order_info['level'] ;
    ...
    public function doSellTotalOrderByStop(){
        ...
        $total_buy_double_num = $this->getTotalNumFromList($this->buy_double_order_list) ;
        if($total_buy_double_num >0){
            // 对手是反向的
            $otype = $type =='up' ? 4:3 ;
            //直接止损，直接以对手价进行卖出
            $obj = new SwapApi($this->returnConfig());
            ...
            $order_type = 0 ;
            $res = $obj->takeOrder($client_oid, $instrumentId, $otype, 0, $total_buy_double_num, $match_price, $leverage, $order_type);
            $this->addLog('DO_SELL_STOP_1301',0,$res);
            ...
            if(!$error_code){
                ...
                // 新增记录
                $buy_order_info = $this->getLastedCompleteOrderByOrderList($this->buy_double_order_list);
                $buy_order_id = $buy_order_info ? $buy_order_info['order_id']: 0;
                $sell_double_table_name = $this->sell_double_table_name ;
                $orginal_group_id = $buy_order_info['group_id'] ;
                return $this->addRowData($coin,$order_info,$buy_order_id,0,0,'',$sell_double_table_name,$orginal_group_id);
    ...
    public function doSellByOrderList(){
        ...
        $buy_order_level = $buy_order_info['level'];

        $buy_double_order_info = $this->getLastedOrderByOrderList($this->buy_double_order_list);

        $buy_double_order_level =  $buy_double_order_info['level'];

        $sell_price = 0 ;
        if($this->type =='up'){

            if($buy_order_level > $buy_double_order_level){
                $total_price = 0 ;
                $total_num = 0 ;
                foreach($this->buy_order_list as $v){
                    $total_price += $v['filled_qty']*$v['price_avg'];
                    $total_num += $v['filled_qty'] ;
                }
                ...
                if($total_num > 0){
                    $price_avg = $total_price/$total_num ;
                    $sell_price = $price_avg*(1 + $this->earn_percent) ;
                }
        ...
        if($this->type =='up'){
            $sell_type = 'up';
            $sell_double_type = 'down' ;
        }else{
            $sell_type = 'down';
            $sell_double_type = 'up' ;
        }

        $sell_order_info = $this->checkIsSell($this->sell_table_name,$buy_order_info['order_id'],1);
        $sell_double_order_info = $this->checkIsSell($this->sell_double_table_name,$buy_double_order_info['order_id']);
        ...
        if(!$sell_order_info){
            ...
            if($buy_order_level <=$buy_double_order_level){
                ...
                /*
                $maker_order_type = 1 ;
                $res = $obj->takeOrder($client_oid, $instrumentId, $otype, $sell_price, $buy_num, $match_price, $leverage, $maker_order_type);
                $error_code = isset($res['error_code']) ? $res['error_code'] : '-1';
        ...
        if(!$sell_double_order_info){
            ...
            $otype = $sell_double_type == 'up' ? 3 : 4 ;

            $buy_double_num = $this->getTotalNumFromList($this->buy_double_order_list);
            ...
            $is_duishou = false;
            //判断当前购买层级少的时候能够立即成交
            if($buy_double_order_level <=$buy_order_level){
                /**
                $maker_order_type = 1 ;
                $res = $obj->takeOrder($client_oid, $instrumentId, $otype, $sell_price, $buy_double_num, $match_price, $leverage, $maker_order_type);
                $error_code = isset($res['error_code']) ? $res['error_code'] : '-1';
                if(!$error_code){
                $order_id = $res['order_id'] ;
                //  查询订单状态信息
                ...
                // 新增记录
                $sell_double_table_name = $this->sell_double_table_name ;
                $this->addRowData($coin,$order_info,$buy_double_order_info['order_id'],0,0,'',$sell_double_table_name,$group_id);
            ...
            if($allowed_sell){
                ...
                $res = $obj->takeOrder($client_oid, $instrumentId, $otype, $sell_price, $buy_double_num, $match_price, $leverage, $order_type);

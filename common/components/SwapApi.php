...
//        return $this->request(self::SWAP_COIN_ACCOUNT.$symbol, [], 'GET');
    ...
    // 账单流水查询 默认第一页 100条
    public function getLedger($symbol,$limit=100)
    {
        $limit = $limit > 100 ? 100:$limit ;
        $url = self::SWAP_LEDGER.$symbol.'/ledger' ;
        $url .= '?limit='.$limit ;
    ...
    // 账单流水查询 默认第一页 100条
    public function getLedgerByType($symbol,$type=1,$limit=100)
    {
        $limit = $limit > 100 ? 100:$limit ;
        $url = self::SWAP_LEDGER.$symbol.'/ledger' ;
        $url .= '?limit='.$limit.'&type='.$type ;
        return $this->request($url, [], 'GET');
    }

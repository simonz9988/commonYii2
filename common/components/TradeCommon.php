...
class TradeCommon {
    ...
    public static $base_coin = 'USD';

    public static $base_buy_num = 6; //最开始购买数量
    // 盈利比例
    ...
    private function returnConfig(){
        $config=[
            "apiKey"=>"b1016adb-1fd7-48e4-94d8-6e51372a3267",
            "apiSecret"=>"8190C7F1B9EFB8D3E7E898E080FA78AD",
    ...
    public function returnAllTableName($coin =''){
        ...
        $all_table_name = ['','sea_okex_buy_up_order','sea_okex_buy_down_order','sea_okex_sell_up_order','sea_okex_sell_down_order'];

        if($coin=='xrp') {
            $all_table_name = ['', 'sea_okex_buy_up_order_xrp', 'sea_okex_buy_down_order_xrp', 'sea_okex_sell_up_order_xrp', 'sea_okex_sell_down_order_xrp'];
        }else if($coin =='btc'){
            $all_table_name = ['','sea_okex_buy_up_order_btc','sea_okex_buy_down_order_btc','sea_okex_sell_up_order_btc','sea_okex_sell_down_order_btc'];
        }else if($coin =='eth'){
            $all_table_name = ['','sea_okex_buy_up_order_eth','sea_okex_buy_down_order_eth','sea_okex_sell_up_order_eth','sea_okex_sell_down_order_eth'];
        }
     ...
     * @return mixed
     */
    public function addMarkRecord($coin,$admin_user_id=1){
        ...
        if(!$sell_info || $sell_info['order_id'] !=0){
            ...
            $sell_add_data['admin_user_id'] =  $admin_user_id ;
    ...
    public function checkIsSell($sell_table_name,$order_id){
        ...
        $params['cond'] = 'buy_id=:buy_id AND state in('.implode(',',$status_arr).')';
    ...
    public function dealSellTradeBySuccessBuyAndSellOrderInfo($type,$buy_order_info,$sell_order_info,$buy_table_name,$sell_table_name){
        ...
        if(in_array($service_sell_order_state,[1,2,3,4])){
            ////-2:失败-1:撤单成功0:等待成交1:部分成交2:完全成交3:下单中4:撤单中
            $model = new OkexOrder();
            $sell_update_data['state'] = $service_sell_order_state ;
            $sell_update_data['modify_time'] = date('Y-m-d H:i:s') ;
    ...
    public function getBuySizeByBuyOrderInfo($buy_order_info){
        ...
        if($level ==1){
            return 2*self::$base_buy_num ;
        }else if($level ==2){
            return 3*self::$base_buy_num ;
        }else if($level ==3){
            return 5*self::$base_buy_num ;
        }else if($level ==4){
            return 8*self::$base_buy_num ;
        }else if($level ==5){
            return 13*self::$base_buy_num ;
        }else{
            return 2*self::$base_buy_num ;
        }
    }
     ...
     * @return mixed
     */
    public function getPositionFromService($coin){

        $instrumentId = $this->returnInstrumentId($coin) ;
        $obj = new SwapApi($this->returnConfig());
        $res = $obj->getSpecificPosition($instrumentId);

        return isset($res['holding'][0]) ? $res['holding'][0] : [] ;
    }

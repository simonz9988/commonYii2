...
class DealAftermarket
{
     ...
     * 相当于之前中台处理失败  callback/confirmRefund  -> aftermarket::oms_refund_fail
     */
    public static function aftermarket_confirm_fail_from_trade_in($data_detail=array(),$msg=''){
        ...
        $obj->baseUpdate('sdb_trade_in_after_market_doc',$doc_update_data,'id=:id',[":id"=>$data_detail['id']]);
    ...
    public function aftermarket_refund_confirm_success($data_detail=array(),$asmOrderNo,$originalOrderNo){
        ...
        if(in_array($after_market_doc_data['after_market_type'] ,array(1,2,3,5))){
            ...
            //直接退款更新订单商品状态 由于退款现在是全部退款
            //if($after_market_doc_data['after_market_type'] ==1){
                $obj->afterDirectRefund($order_id);
            //}
    ...
    public function aftermarket_return_close($data_detail = array(),$asmOrderNo,$originalOrderNo){
        ...
        */
        ...
        $doc_update_data = ['dispose_status'=>2,'dispose_time'=>$current_time,'dispose_idea'=>'由于退货单关闭,售后单也随之关闭','doc_log'=>$doc_log] ;
        $obj->baseUpdate('sdb_after_market_doc',$doc_update_data,'id=:id',[':id'=>$data_detail['id']]);
    ...
    public function aftermarket_exchange_close($data_detail = array(),$asmOrderNo,$originalOrderNo){
        ...
        //step3 更新售后单的相关字段
        $doc_update_data = ['dispose_status'=>2,'dispose_time'=>$current_time,'dispose_idea'=>'由于换货单关闭,售后单也随之关闭','doc_log'=>$doc_log] ;
        $obj->baseUpdate('sdb_after_market_doc',$doc_update_data,'id=:id',[':id'=>$data_detail['id']]);
    ...
    public function aftermarket_repair_close($data_detail = array(),$asmOrderNo,$originalOrderNo){
        ...
        //step3 更新售后单的相关字段
        $doc_update_data = ['dispose_status'=>2,'dispose_time'=>$current_time,'dispose_idea'=>'由于维修单关闭,售后单也随之关闭','doc_log'=>$doc_log] ;
        $obj->baseUpdate('sdb_after_market_doc',$doc_update_data,'id=:id',[':id'=>$data_detail['id']]);
    ...
    public function close_after_market_doc($after_market_doc_info=array(),$asmOrderNo,$originalOrderNo,$status='',$msg=''){
        ...
        if($status =='PROCESSING' || $status=='WAITING' || $status=='FINISHED'){
            return $rst ;
        }
        ...
        if($after_market_doc_info ){
            $type = $after_market_doc_info['after_market_type'];
            if($type ==1){
                //退款
                $rst = $this->aftermarket_refund_confirm_success($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                $rst = $this->aftermarket_refund_close($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                //售后废除处理发货状态
                //$this->update_order_distribution_status($after_market_doc_info['order_id']);
            }else if($type ==2){

                //退货
                $rst = $this->aftermarket_return_confirm_success($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                $rst = $this->aftermarket_return_close($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                //售后废除处理发货状态
                //$this->update_order_distribution_status($after_market_doc_info['order_id']);
            }else if($type ==3){
                //换货
                $rst = $this->aftermarket_exchange_confirm_success($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                $rst = $this->aftermarket_exchange_close($after_market_doc_info,$asmOrderNo,$originalOrderNo);
            }else if($type ==4){
                //维修  该情况下直接完成售后就OK
                $rst = $this->aftermarket_repair_close($after_market_doc_info,$asmOrderNo,$originalOrderNo);
            }else if($type ==5){
                //退差价
                $rst = $this->aftermarket_refund_confirm_success($after_market_doc_info,$asmOrderNo,$originalOrderNo);
                $rst = $this->aftermarket_refund_close($after_market_doc_info,$asmOrderNo,$originalOrderNo);
    ...
    public function trade_in_aftermarket_close($doc_info,$data,$log_info){
        ...
        if($voucher_record_rst['code']==1){
            ...
            if($mobile) {
                ...
                $msg = '小科已收到您寄回的机器，以旧换新代金券已发放到您的科沃斯官网商城账号中，有效期至' . $time_str . '，请您注意查收。感谢您对小科的支持，祝生活愉快！';
    ...
    public function dealTradeInCallback($data=array()){
        ...
        if(!$receive_info){
            ...
            $rst['msg'] = '物料号在官网不存在';
            $after_market_doc_info = $doc_obj->getDocInfoByAsmNo($asmOrderNo);
            $this->addTradeInResponseLog($after_market_doc_info,$data,$rst);//添加日志
            return $rst ;
        }
     ...
     * @param $shopResponseData 后台处理数据
     */
    public function addTradeInResponseLog($doc_info,$asmResponseData,$shopResponseData){

        $params = json_encode($asmResponseData);
        $doc_info = json_encode($doc_info);
        $shop_params = json_encode($shopResponseData);
        $create_time = date('Y-m-d H:i:s');

        $add_data = compact('params','doc_info','shop_params','create_time');
        //当BaseModel来用
        $obj = new AfterMarketDoc();
        $obj->baseInsert('task_trade_in_callback_log',$add_data,'db_log');
    }

...
/**
 * This is the model class for table "sea_v6399_shouyou_app".
 *
 * @property string $id
 ...
 */
class V6399ShouyouApp extends \common\models\BaseModel
{
    /**
     * @inheritdoc
     */
    ...
    public function initAppContent($id){
        ...
        $new_static_domain = 'http://static.s115.cn';
        ...
        if (!$cover_img_url) {
            ...
            $this->baseUpdate($this->tableName(), $updateData, $where_str, $where_arr,'db_ms115');
    ...
    public function dealIconCoverImage($id){
        ...
        if($info){
            ...
            $this->baseUpdate($this->tableName(),array('icon_img_url'=>$new_url,'is_deal_cover_images'=>1),'id=:id',array(':id'=>$info['id']),'db_ms115');
        }
    }
    ...
    public function dealJietu($id){
        ...
        if($info){
            ...
            $this->baseUpdate($this->tableName(),array('jietu'=>$new_jietu,'is_deal_jietu'=>1),'id=:id',array(':id'=>$info['id']),'db_ms115');
            var_dump($info['id']);
        }
    }
    ...
    public function doPublish(){
        ...
        if($list){
            foreach($list as $info){
                ...
                $this->baseUpdate($this->tableName(),$update_data,'id=:id',array(':id'=>$info['id']),'db_ms115');
            }
        }
    }
    ...
    public function getTopList(){
        ...
        $params['args'] = [':status'=>'enable',':is_open'=>1];
        $params['fields'] = 'id,icon_img_url,title,daxiao';
        $params['orderby'] = 'id desc ';
        $params['limit'] = '24';
        return $this->findAllByWhere($this->tableName(),$params,self::getDb());
    }

    public function getListByPage($curr_page){
        $params['cond'] = 'status =:status and is_open =:is_open';
        $params['args'] = [':status'=>'enable',':is_open'=>1];
        $params['fields'] = 'id,icon_img_url,title,publish_time,daxiao';
        $params['orderby'] = 'id desc ';
        $page_num = 12;

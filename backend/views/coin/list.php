...
<div class="breadcrumbs" id="breadcrumbs">
    <div class="row">
        ...
        <div class="col-md-4 text-right">
            <a href="/coin/edit" class="btn btn-primary btn-sm no-border"><i class="ace-icon glyphicon glyphicon-plus"></i> 新增</a>
        </div>
    </div>
</div>
<!-- PAGE CONTENT BEGINS  -->
    ...
    <div class="page-header">
        ...
        <div class="row">
            <div class="col-xs-12 page-search">
                <form class="form-inline" id="shippingOrderSearchForm" action="/coin/list" method="get">
                    ...
                    <div class="form-group">
                        ...
                        <select name="platform">
                            ...
                            <?php foreach($bit_all_platform as $k=>$v):?>
                                <option value="<?=$k?>" <?=$searchArr && $searchArr['platform']!= '' && $searchArr['platform'] ==$k?'selected="selected"':''?>><?=$v?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        <label>盈利类型：</label>
                        <select name="earn_type">
                            <option value="">请选择</option>
                            <?php foreach($earn_type_list as $k=>$v):?>
                                <option value="<?=$k?>" <?=$searchArr && $searchArr['earn_type']!= '' && $searchArr['earn_type'] ==$k?'selected="selected"':''?>><?=$v?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <tbody>
                        <?php if($list):?>
                            ...
                            <tr>
                                ...
                                <td><?=$v['alias']?></td>
                                <td><?=$v['unique_key']?></td>
                                <td><?=$v['earn_type_name']?></td>
                                <td><?=$v['create_time']?></td>
                                <td>
                                    <a class="btn btn-primary btn-xs" href="<?=url('/coin/edit?id='.$v['id'])?>"> <i class="ace-icon fa fa-edit bigger-100"></i> 修改</a>
                                    <a class="btn btn-xs btn-danger cus-btn-del" href="javascript:void(0);" onclick="delModel('/coin/del?id=<?=$v['id']?>')">

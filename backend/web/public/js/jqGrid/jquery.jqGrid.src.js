...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			multiselect: false,
			multikey: false,
			hidemouseselect: true,//自定义的  是否隐藏鼠标选择功能 by xzhu
			rangemultiselect: false,//自定义的 by xzhu
			editurl: null,
			search: false,
			caption: "",
			hidegrid: true,
		...
		//var sortkeys = ["shiftKey","altKey","ctrlKey"], by xzhu
		var sortkeys = ["ctrlKey"],
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			//modify by xzhu start
			if ( !ts.p.multikey ) {
				if(ts.p.multiselect && ts.p.multiboxonly) {
					if(ts.p.rangemultiselect && e["shiftKey"]){
						$(ts).jqGrid("setSelection",ri,true,e);
					}else{
				...
				if(e[ts.p.multikey]) {
					$(ts).jqGrid("setSelection",ri,true,e);
				} else if(ts.p.multiselect && ts.p.multiboxonly) {
					if(ts.p.rangemultiselect && e["shiftKey"]){
						$(ts).jqGrid("setSelection",ri,true,e);
					}else{
						if(scb){$(ts).jqGrid("setSelection",ri,true,e);}
		...
		//这个会把鼠标选择功能隐掉  xzhu
		if(ts.p.multikey && ts.p.hidemouseselect){
			if( $.jgrid.msie) {
...
window.jqGridUtils = {
	...
	function hackHeight(gridId) {
		$(gridId + '_frozen tr').slice(1).each(function() {
			...
			var normalHeight = parseFloat($(gridId + ' #' + rowId).find('td:first').height());
			// 如果冻结的列高度小于未冻结列的高度则hack之
			if (frozenTdHeight < normalHeight) {
				$('td', this).height(normalHeight+0.6);
//				$('td', this).each(function() {
//
//					/*
//					 浏览器差异高度hack
//					 */
//					var space = 0.6; // opera默认使用0就可以
////                        if (browser.isChrome()) {
////                            space = 0.6;
...
//					//	$(this).attr('style', $(this).attr('style') + ";height:" + (normalHeight) + "px !important");
//					//}
//				});
			}else{
				console.log("frozenTdHeight:"+frozenTdHeight);

<?php
namespace backend\extensions\weixinmini\lib;
use Yii;
use backend\extensions\weixinmini\lib\WxPayException;
use backend\extensions\weixinmini\lib\WxPayConfig;
use backend\extensions\weixinmini\lib\WxPayDataBase;
...
class WxPayApi
{
	...
	public static function refund($inputObj, $timeOut = 6)
	{
		...
		// 检测必填参数
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
            return ['code' => 0, 'msg' => "微信退款申请接口中，out_trade_no、transaction_id至少填一个！"];
		}else if(!$inputObj->IsOut_refund_noSet()){
            return ['code' => 0, 'msg' => "微信退款申请接口中，缺少必填参数out_refund_no！"];
		}else if(!$inputObj->IsTotal_feeSet()){
            return ['code' => 0, 'msg' => "微信退款申请接口中，缺少必填参数total_fee！"];
		}else if(!$inputObj->IsRefund_feeSet()){
            return ['code' => 0, 'msg' => "微信退款申请接口中，缺少必填参数refund_fee！"];
		}else if(!$inputObj->IsOp_user_idSet()){
            return ['code' => 0, 'msg' => "微信退款申请接口中，缺少必填参数op_user_id！"];
		}
	...
	public static function refundQuery($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsOut_refund_noSet() &&
            ...
            return ['code' => 0, 'msg' => "退款查询接口中，out_refund_no、out_trade_no、transaction_id、refund_id四个参数必填一个！"];
		}
		$inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
		$inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($data){
            ...
            Yii::$app->CommonLogger->logError("微信退款curl出错：".json_encode($error_info,JSON_UNESCAPED_UNICODE));

...
namespace backend\extensions\weixinmini;

use Yii;
use backend\extensions\weixinmini\lib\WxPayApi;
use backend\extensions\weixinmini\lib\WxPayConfig;
use backend\extensions\weixinmini\lib\WxPayRefund;
use backend\extensions\weixinmini\lib\WxPayRefundQuery;
use backend\extensions\weixinmini\lib\WxPayOrderQuery;
...
class weixinmini{
    ...
    public function refund($trade_no, $total_amount, $refund_amount, $refund_trade_no, $refund_reason = '')
    {
        ...
        if(!$response || $response['return_code'] != 'SUCCESS' || $response['result_code'] != 'SUCCESS'){
            ...
            Yii::$app->CommonLogger->logError("微信小程序退款返回失败：".json_encode($error_info,JSON_UNESCAPED_UNICODE));
        }
    ...
    {
        require_once(__DIR__ . '/lib/WxPayDataBase.php');

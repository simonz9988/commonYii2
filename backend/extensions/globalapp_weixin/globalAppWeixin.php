...
namespace backend\extensions\globalapp_weixin;

use Yii;
use backend\extensions\globalapp_weixin\lib\WxPayApi;
use backend\extensions\globalapp_weixin\lib\WxPayConfig;
use backend\extensions\globalapp_weixin\lib\WxPayRefund;
use backend\extensions\globalapp_weixin\lib\WxPayRefundQuery;
use backend\extensions\globalapp_weixin\lib\WxPayOrderQuery;

// 设置时区
ini_set('date.timezone','Asia/Shanghai');

/**
 * @class globalAppWeixin
 * @brief 支付宝接口
 */
class globalAppWeixin{
    ...
    public function refund($trade_no, $total_amount, $refund_amount, $refund_trade_no, $refund_reason = '')
    {
        ...
        if(!$response || $response['return_code'] != 'SUCCESS' || $response['result_code'] != 'SUCCESS'){
            ...
            Yii::$app->CommonLogger->logError("globalApp微信退款返回失败：".json_encode($error_info,JSON_UNESCAPED_UNICODE));
        }

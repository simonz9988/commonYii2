...
namespace backend\extensions\weixin;

use Yii;
use backend\extensions\weixin\lib\WxPayApi;
use backend\extensions\weixin\lib\WxPayConfig;
use backend\extensions\weixin\lib\WxPayRefund;
use backend\extensions\weixin\lib\WxPayRefundQuery;
use backend\extensions\weixin\lib\WxPayOrderQuery;
...
class weixin{
    ...
    public function refund($trade_no, $total_amount, $refund_amount, $refund_trade_no, $refund_reason = '')
    {
        ...
        if(!$response || $response['return_code'] != 'SUCCESS' || $response['result_code'] != 'SUCCESS'){
            ...
            Yii::$app->CommonLogger->logError("微信退款返回失败：".json_encode($error_info,JSON_UNESCAPED_UNICODE));
        }

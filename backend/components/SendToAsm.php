...
class SendToAsm
{
    ...
    //获取售后类型的中文释义
     private static function getAftermarketProblemTypeStr($type,$after_market_info){
        $rst = '';
    ...
    public static function sendDataByCurl($url='',$post=array(), $data_type=''){
        ...
        //curl调用统一方法
        $tmp_content		= curlGo($url,$post,false,null,$data_type);
    ...
    public static function getTasks(){
        ...
        if($info){
            foreach($info as $v){
                ...
                $row_info = $task_obj->findOneByWhere('sdb_asm_task',['cond'=>'id=:id','args'=>[':id'=>$v['id']]]);
                //设置返回结果
                $params = unserialize($row_info['params']);
                if($params){

                    $after_market_doc_id = isset($row_info['after_market_doc_id'])?$row_info['after_market_doc_id']:0;
    ...
    public static function returnApplyWorkOrderOmsData($after_market_doc_id=0,$order_info=array()){
        ...
        if($order_info){
            ...
            $rst['problemTypeStr'] = self::getAftermarketProblemTypeStr($orderType,$after_market_info);//传问题类型，非必填
    ...
    public static function sendApplyDataToAsm($after_market_doc_id,$order_info){
        ...
        if($oms_return_code !='0000'){
            ...
            //记录错误日志
            $log_data['url'] = $create_asm_order_url;
            $log_data['response_data'] = $oms_return_arr;
            Yii::$app->CommonLogger->logErrorConsole("发送售后申请工单数据给售后中心返回失败：".json_encode($log_data));
            return $rst ;
        }
    ...
    public static function doCancel($after_market_doc_id){
        ...
        //判断新版售后任务是否发送请求到中台
        $task_params['cond'] = ' business_id = :business_id and status ="NOPUSH" ';
        $task_params['args'] = [':business_id'=>$after_market_doc_id];
        $task_info = $task_obj->findOneByWhere('sdb_push_task',$task_params);

        if($task_info){
            //更新为关闭
            $task_obj->baseUpdate('sdb_push_task',['status'=>'CLOSED'],'id=:id',[':id'=>$task_info['id']]);
        }
    ...
    public static function sendCancelWorkOrderToOms($after_market_doc_id=0,$cancel_reason=''){
        ...
        if($send_data) {
            ...
            if ($oms_return_code != '0000') {
                ...
                //记录错误日志
                $log_data['url'] = $cancel_asm_order_url;
                $log_data['response_data'] = $oms_return_arr;
                Yii::$app->CommonLogger->logError("发送取消工单请求返回失败：".json_encode($log_data));
        ...
        if($info){
            foreach($info as $v){
                ...
                if($params){
                    ...
                    $after_market_doc_id = isset($row['after_market_doc_id'])?$row['after_market_doc_id']:0;
                    $postData  = isset($params['postData'])?$params['postData']:array();
                    $userInfo  = isset($params['userInfo'])?$params['userInfo']:array();
                    $task_info[] = array(
                        'after_market_doc_id'=>$after_market_doc_id,
    ...
    public static function sendApplyDataToAsmFromTradeIn($after_market_doc_id){
        ...
        if($oms_return_code !='0000'){
            ...
            $log_data['url'] = $create_asm_order_url;
            $log_data['response_data'] = $oms_return_arr;
            Yii::$app->CommonLogger->logError("发送售后申请工单数据给手中心返回失败：".json_encode($log_data));
     ...
     * 发送修复以旧换新的售后任务
     */
    public  static function doFixSendFromTradeIn($trade_in_apply_id=0){

        $trade_in_apply_obj = new TradeInApply();
        $trade_in_apply_info = $trade_in_apply_obj->findOneByWhere('sdb_trade_in_apply',['cond'=>'id=:id','args'=>[':id'=>$trade_in_apply_id]]);

        if($trade_in_apply_info) {

            $now = date('Y-m-d H:i:s');
            //step1 常见售后单信息
            $user_id = $trade_in_apply_info['user_id'];
            $data['user_id'] = $user_id;
            $data['username'] = $trade_in_apply_info['username'];
            $data['trade_in_id'] = $trade_in_apply_info['trade_in_id'];
            $data['trade_in_apply_id'] = $trade_in_apply_id;
            $data['delivery_code'] = isset($trade_in_apply_info['delivery_code']) ? $trade_in_apply_info['delivery_code'] : 0;
            $data['freight_code'] = isset($trade_in_apply_info['freight_code']) ? $trade_in_apply_info['freight_code'] : '';

            $data['apply_goods_name'] = isset($trade_in_apply_info['apply_goods_name']) ? $trade_in_apply_info['apply_goods_name'] : '';
            $data['apply_materiel_no'] = isset($trade_in_apply_info['apply_materiel_no']) ? $trade_in_apply_info['apply_materiel_no'] : '';
            $data['estimated_price'] = isset($trade_in_apply_info['estimated_price']) ? $trade_in_apply_info['estimated_price'] : 0;
            $data['sn'] = isset($trade_in_apply_info['sn']) ? $trade_in_apply_info['sn'] : '';

            $data['pay_type'] = 'cash_coupon';//支付方式在依旧换新情况下是固定的
            $data['contact'] = isset($trade_in_apply_info['contact']) ? $trade_in_apply_info['contact'] : '';
            $data['mobile'] = isset($trade_in_apply_info['mobile']) ? $trade_in_apply_info['mobile'] : '';
            $data['province'] = isset($trade_in_apply_info['province']) ? $trade_in_apply_info['province'] : 0;
            $data['city'] = isset($trade_in_apply_info['city']) ? $trade_in_apply_info['city'] : 0;
            $data['area'] = isset($trade_in_apply_info['area']) ? $trade_in_apply_info['area'] : 0;
            $data['address'] = isset($trade_in_apply_info['address']) ? $trade_in_apply_info['address'] : 0;
            $data['audit_status'] = 0;
            $data['audit_idea'] = '';
            $data['dispose_status'] = 0;
            $data['is_send_to_oms'] = 0;
            $data['dispose_time'] = '0000-00-00 00:00:00';
            $data['dispose_idea'] = '';
            $data['create_time'] = $now;
            $data['update_time'] = $now;

            $doc_id = $trade_in_apply_obj->baseInsert('sdb_trade_in_after_market_doc', $data);

            //创建task
            $service_key = 'send_aftermarket_to_asm';
            $task_data['after_market_doc_id'] = $doc_id;
            $task_data['name'] = '发送售后请求到售后中心';
            $task_data['service_key'] = $service_key;

            $params['after_market_doc_id'] = $doc_id;
            $params['postData'] = $trade_in_apply_info;
            ...
            $task_data['update_time'] = date('Y-m-d H:i:s');
            $task_data['status'] = 'SENDING';
            $task_id = $trade_in_apply_obj->baseInsert('sdb_trade_in_asm_task', $task_data);

            //获取售后任务信息

            $userInfo  = isset($params['userInfo'])?$params['userInfo']:array();
            $task_list[] = array(
                'after_market_doc_id'=>$doc_id,
                'postData'=>$trade_in_apply_info,
                'userInfo'=>$userInfo,
                'id'=>$task_id,
            );
            ...
            if ($task_list) {
                ...
                foreach ($task_list as $v) {
                    ...
                    //step1 将售后信息发往售后中心 并更新相关表数据
                    $send_rst = self::sendFixApplyDataToAsmFromTradeIn($after_market_doc_id);
     ...
     * {"code":"0000","msg":"操作成功","data":{"asmOrderNo":"701466390703933004006","originalOrderNo":"118143003026"}}
     */
    public static function sendFixApplyDataToAsmFromTradeIn($after_market_doc_id){
        ...
        //step2 发送给中台的数据
        $send_data = self::returnFixApplyWorkOrderOmsDataFromTradeIn($after_market_doc_id);
        $create_asm_order_url = self::getCreateAsmOrderUrl();
        $oms_return_info = self::sendDataByCurl($create_asm_order_url,$send_data,'json');
     ...
     * 	OLD_FOR_NEW：以旧换新
     */
    public static function returnFixApplyWorkOrderOmsDataFromTradeIn($after_market_doc_id=0){
        $rst = array() ;
        ...
        if($after_market_doc_id){
            ...
            $channel_url = SHOP_ADMIN_URL ;
            $work_order_callback_url = $channel_url.'/ec-callback/deal-fix-trade-in-aftermarket';//工单关闭回调地址
            $rst['callBackNotifyUrl'] = $work_order_callback_url;//回调地址 工单完成关闭
            $rst['validateCode'] = self::createValidateCodeFromTradeIn($after_market_info);//发往中台唯一标识符
            $rst['orderSource'] = 'asm_order_source_ecovacs';
            $rst['contactInfo'] = self::getTradeInContactInfo($after_market_info);

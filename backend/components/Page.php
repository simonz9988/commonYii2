...
class Page {
    ...
    public function showResForBackstage() {
        ...
        $result = array(
        				...
        				'firstpage' => 	array('row' => '0', 'href' => $url.$p."=1"),
        				'endpage' 	=> 	array('row' => 0, 'href' => $url.$p."=".$this->totalPages),
        				'prev' 		=> 	array('row' => '', 'href' => ''),
        				'next' 		=> 	array('row' => '', 'href' => ''),
        				'page' 		=>	array(),
                        'baseurl'   =>  $parse['path'],
        ...
        if ($upRow>0) {
            $result['prev'] = array('row' => $upRow, 'href' => $url.$p."=".$upRow);
        }
    
        if ($upRow>0) {
            $result['firstpage'] = array('row' => $upRow, 'href' => $url.$p."=1");
        }

        if ($this->totalPages > $this->nowPage) {
            $result['endpage'] = array('row' => $this->totalPages, 'href' => $url.$p."=".$this->totalPages);
        }

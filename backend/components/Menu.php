...
class Menu
{
    ...
    public static function getCasUserMenu($username){
        ...
        if($result['code'] != '0000'){
            ...
            Yii::$app->CommonLogger->logError("中台获取用户菜单返回失败：".json_encode($log_data));
        }
    ...
    public static function getCasPrivilegeWithMenu(){
        ...
        $result = curlGo($url);
        $result = json_decode($result, true);
        //todo 增加异常捕获 code = '0000'
        if($result['code'] != '0000'){
            $log_data['url'] = $url;
            $log_data['response_data'] = $result;
            Yii::$app->CommonLogger->logError("中台获取带菜单层次结构的权限列表返回失败：".json_encode($log_data));
        }
